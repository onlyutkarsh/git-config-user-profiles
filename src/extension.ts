...
export async function activate(context: ExtensionContext) {
	try {
		...
		context.subscriptions.push(
			commands.registerCommand(constants.CommandIds.CREATE_USER_PROFILE, cmd.createUserProfile)
		);
		context.subscriptions.push(
			commands.registerCommand(
				constants.CommandIds.SYNC_VSC_PROFILES_WITH_GIT_CONFIG,
				cmd.syncVscProfilesWithGitConfig
			)
		);
		context.subscriptions.push(
			commands.registerCommand(constants.CommandIds.DELETE_USER_PROFILE, cmd.deleteUserProfile)
		);
		context.subscriptions.push(
			commands.registerCommand(constants.CommandIds.EDIT_USER_PROFILE, cmd.editUserProfile)
		);
		context.subscriptions.push(
