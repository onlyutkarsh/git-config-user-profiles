...
import { deleteVscProfile, getVscProfiles, saveVscProfile } from './config';
...
export async function deleteUserProfile() {
	...
	const pickedProfile = await window.showQuickPick<Profile>(
		...
		}),
		{
			...
			matchOnDetail: false,
			ignoreFocusOut: true,
			placeHolder: 'Select a user profile to delete. '
		}
	);
	...
	if (pickedProfile) {
		...
		await sgit(workspaceFolder).addConfig('user.name', '');
		await sgit(workspaceFolder).addConfig('user.email', '');
		await sgit(workspaceFolder).addConfig('user.signingkey', '');
		await deleteVscProfile(pickedProfile);
	}
	return undefined;
}
...
export async function getUserProfile(fromStatusBar = false, notProfileSwitch = true): Promise<Profile> {
	...
	const selectedVscProfile: Profile =
		selectedProfileInVscConfig.length > 0 ? selectedProfileInVscConfig[0] : emptyProfile;

	//TODO: Show error if the user deliberately deletes the username or email property from config
	if (
		selectedVscProfile.label === undefined ||
		selectedVscProfile.userName === undefined ||
		selectedVscProfile.email === undefined
	) {
	...
	if (validatedWorkspace.isValid && validatedWorkspace.folder) {
		...
		configInSync =
			!util.isNameAndEmailEmpty(currentGitConfig) &&
			util.hasSameNameAndEmail(currentGitConfig, selectedVscProfile);
	}
	...
	if (fromStatusBar) {
		if (profilesInVscConfig.length === 0) {
			...
			const selected = await window.showInformationMessage(
				'No user profiles defined. Do you want to define one now?',
				'Yes',
				'No'
			);
			if (selected === 'Yes') {
		...
		if (selectedProfileInVscConfig.length === 0) {
			...
			const message = configInSync
				? `Git config is already in sync with profile '${util.trimLabelIcons(
						selectedVscProfile.label
					)}'. What do you want to do?`
				: `Git config is not using this profile. Do you want to use profile '${util.trimLabelIcons(
						selectedVscProfile.label
					)}' for this repo? (user: ${selectedVscProfile.userName}, email: ${selectedVscProfile.email}) `;
		...
		if (response === 'Delete user profile') {
			await deleteUserProfile(selectedVscProfile);
			return selectedVscProfile;
		}
	}
	return emptyProfile;
...
export async function syncVscProfilesWithGitConfig(): Promise<void> {
	...
	await Promise.all(
		vscProfiles.filter((vscProfile) => vscProfile.selected).map(async (vscProfile) => {
	...
	const correspondingVscProfile = vscProfiles.filter((vscProfile) =>
		util.hasSameNameAndEmail(vscProfile, gitProfile)
	);
