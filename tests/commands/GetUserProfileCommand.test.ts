...
import { ProfileStatusBar, StatusBarStatus } from "../../src/controls/profileStatusBar";
...
describe("GetUserProfileCommand - status bar visibility", () => {
  ...
  describe("no active editor (e.g. VS Code just started, all tabs closed)", () => {
    test("should show status bar with friendly tooltip — not hide it (fixes #120)", async () => {
      ...
      expect(mockUpdateStatus()).toHaveBeenCalledWith(undefined, undefined, StatusBarStatus.Normal, "Open a file from a git repository");
    });
    ...
    test("should show status bar when no workspace folder message is returned", async () => {
      ...
      expect(mockHide()).not.toHaveBeenCalled();
      expect(mockUpdateStatus()).toHaveBeenCalledWith(undefined, undefined, StatusBarStatus.Normal, "No workspace folder found.");
    });
    ...
    test("should show status bar when file is not part of a workspace folder", async () => {
      ...
      expect(mockHide()).not.toHaveBeenCalled();
      expect(mockUpdateStatus()).toHaveBeenCalledWith(undefined, undefined, StatusBarStatus.Normal, "This file is not part of a workspace folder.");
    });
  });
  ...
  describe("confirmed not a git repository", () => {
    ...
    test("should show status bar with tooltip in always mode", async () => {
      ...
      expect(mockHide()).not.toHaveBeenCalled();
      expect(mockUpdateStatus()).toHaveBeenCalledWith(undefined, undefined, StatusBarStatus.Normal, Messages.NOT_A_VALID_REPO);
    });
  });

  describe("non-file scheme (Output panel, Settings, etc.)", () => {
    test("should show an actionable prompt when message is empty (non-file scheme)", async () => {
  ...
  describe("valid git repo with profile", () => {
    test("should show status bar normally when config is in sync", async () => {
      ...
      expect(mockHide()).not.toHaveBeenCalled();
      expect(mockUpdateStatus()).toHaveBeenCalledWith(profile, "/repo/my-project", StatusBarStatus.Normal, undefined, {
        userName: "Work User",
    ...
    test("should show warning when config is out of sync", async () => {
      ...
      expect(mockHide()).not.toHaveBeenCalled();
      expect(mockUpdateStatus()).toHaveBeenCalledWith(profile, "/repo/my-project", StatusBarStatus.Warning, undefined, {
        userName: "Other User",
